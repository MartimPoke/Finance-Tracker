...
const SettingsView: React.FC<SettingsViewProps> = ({ transactions, setTransactions, userProfile, setUserProfile, onLogout }) => {
  ...
  // Sincronizar tempValue quando o campo de edição mudar
  useEffect(() => {
    if (editingField) {
      setTempValue(String(userProfile[editingField] || ''));
    }
  }, [editingField, userProfile]);

  const handleSave = () => {
    if (!editingField) return;

    try {
      console.log(`A guardar campo ${editingField} com valor: ${tempValue}`);
      ...
      if (editingField === 'age') {
        ...
        if (isNaN(num)) throw new Error("A idade deve ser um número válido.");
        finalValue = num;
      }
      ...
      // Feedback opcional de sucesso (podia ser um toast)
      console.log("Perfil atualizado com sucesso no estado local.");
    } catch (err: any) {
      console.error("Falha ao guardar perfil:", err);
      alert(err.message || "Erro inesperado ao guardar. Tente novamente.");
    }
  };
  ...
  const exportToExcel = () => {
    ...
    const wscols = [{ wch: 15 }, { wch: 12 }, { wch: 30 }, { wch: 12 }, { wch: 10 }, { wch: 15 }, { wch: 10 }];
    ws['!cols'] = wscols;
  ...
  const exportToCSV = () => {
    ...
    const rows = transactions.map(t => [t.id, t.date, `"${t.description.replace(/"/g, '""')}"`, t.amount.toString().replace('.', ','), t.type === 'INCOME' ? 'Receita' : 'Despesa', t.method, t.isRecurring ? 'Sim' : 'Não']);
    ...
    <div className="space-y-6">
      <div className="flex flex-col items-center py-6">
        <div className={`w-24 h-24 rounded-full flex items-center justify-center text-3xl mb-4 text-blue-600 font-black shadow-inner transition-colors ${userProfile.isDarkMode ? 'bg-[#1C1F23]' : 'bg-gray-100'}`}>
           {userProfile.name.split(' ').filter(Boolean).map(n => n[0]).join('').toUpperCase().slice(0, 2)}
        </div>
      ...
      <div className={`rounded-[2.5rem] border overflow-hidden transition-colors ${cardClass}`}>
        <div className="p-6">
          ...
          <div className="space-y-1">
            ...
            <SettingRow label="Data Nasc." value={userProfile.birthDate || 'Não definida'} onEdit={() => setEditingField('birthDate')} isDarkMode={userProfile.isDarkMode} />
          </div>
        </div>
        ...
        <div className={`p-6 border-t ${userProfile.isDarkMode ? 'border-[#2A2E33]' : 'border-gray-50'}`}>
          ...
          <div className="space-y-3">
            <button onClick={exportToExcel} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all active:scale-95 ${userProfile.isDarkMode ? 'bg-green-900/20 text-green-400 border border-green-900/30' : 'bg-green-50 text-green-700 border border-green-100'}`}>
              <i className="fa-solid fa-file-excel"></i> Exportar para Excel (.xlsx)
            </button>
            <button onClick={exportToCSV} className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all active:scale-95 ${userProfile.isDarkMode ? 'bg-blue-900/20 text-blue-400 border border-blue-900/30' : 'bg-blue-50 text-blue-600 border border-blue-100'}`}>
              <i className="fa-solid fa-file-csv"></i> Exportar CSV
            </button>
            <button onClick={onLogout} className="w-full flex items-center gap-4 p-4 rounded-2xl font-bold bg-red-50 text-red-600 border border-red-100 active:scale-95">
      ...
      <AnimatePresence>
        {editingField && (
          <motion.div 
            ...
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }} 
            className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end justify-center px-6 pb-12"
          >
            <motion.div 
              initial={{ y: 100 }} 
              animate={{ y: 0 }} 
              exit={{ y: 100 }} 
              className={`w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl ${userProfile.isDarkMode ? 'bg-[#1C1F23]' : 'bg-white'}`}
            >
              <h4 className={`text-xl font-black mb-6 ${userProfile.isDarkMode ? 'text-white' : 'text-gray-800'}`}>
                Editar {editingField === 'birthDate' ? 'Data' : editingField}
              </h4>
              
              <input 
                autoFocus 
                type={editingField === 'age' ? 'number' : editingField === 'birthDate' ? 'date' : 'text'}
                value={tempValue}
              ...
              <div className="flex gap-4">
                <button 
                  ...
                  className={`flex-1 py-4 rounded-2xl font-bold ${userProfile.isDarkMode ? 'bg-[#2A2E33] text-gray-500' : 'bg-gray-100 text-gray-400'}`}
                >
                  Cancelar
                </button>
                <button 
                  onClick={handleSave} 
                  className="flex-1 bg-blue-600 py-4 rounded-2xl font-bold text-white shadow-lg active:scale-95 transition-transform"
                >
                  Salvar
                </button>
              </div>
