...
const Dashboard: React.FC<DashboardProps> = ({ transactions, categories, onAddClick }) => {
    ...
    <div className="space-y-6">
      ...
      >
        <div className="relative z-10">
          <p className="text-gray-400 text-sm font-semibold mb-1">Saldo Total</p>
          ...
          <div className="flex gap-4">
            <button onClick={onAddClick} className="bg-white text-black px-6 py-2.5 rounded-full text-sm font-bold flex items-center gap-2">
              <i className="fa-solid fa-arrow-down-long"></i> Add
            </button>
            <button className="bg-white/10 backdrop-blur-md text-white px-6 py-2.5 rounded-full text-sm font-bold flex items-center gap-2">
              <i className="fa-solid fa-share"></i> Enviar
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-5 rounded-[2rem] border border-gray-100 flex flex-col gap-2">
          ...
          <span className="text-lg font-black text-green-600">+{income.toFixed(2)}€</span>
        </div>
        <div className="bg-white p-5 rounded-[2rem] border border-gray-100 flex flex-col gap-2">
          ...
          <span className="text-lg font-black text-red-600">-{expenses.toFixed(2)}€</span>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-[2.5rem] border border-gray-100">
        <h3 className="text-sm font-bold text-gray-800 mb-4 flex items-center justify-between">
          Gastos Diários (7 dias)
          <span className="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full font-black">Média 45€</span>
        </h3>
      ...
      {/* Quick Transactions list teaser */}
      <div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                   ...
                   <div>
                     <p className="text-sm font-extrabold text-[#191C1F]">{t.description}</p>
